...
async function sendCommandButtons(sock, chatId, persistDir, cat) {
  ...
  // (زي '!قفل / !فتح' أو '!توقف / !تشغيل') - دول مش أمر واحد قابل للإرسال، فزر واحد
  // بس ما يقدر يبعتهم مع بعض (COMMAND_GATES بتوقع نص مطابق لواحد منهم بالظبط). لهيك
  // نفكّهم هون لزرين منفصلين، كل واحد يبعت نص الأمر الحقيقي القابل للمطابقة لحاله.
  const flatButtons = [];
  for (const c of commands) {
    const parts = c.name.split(' / ').map((p) => p.trim()).filter(Boolean);
    parts.forEach((part, idx) => {
      flatButtons.push({ text: part, id: `cmd_${cat}_${c.key}${parts.length > 1 ? `_${idx}` : ''}` });
    });
  }

  const BATCH_SIZE = 3;
  const totalBatches = Math.ceil(flatButtons.length / BATCH_SIZE);

  for (let i = 0; i < totalBatches; i++) {
    const batch = flatButtons.slice(i * BATCH_SIZE, i * BATCH_SIZE + BATCH_SIZE);
    try {
      await sock.sendMessage(chatId, {
        text: 'اضغط على الأمر اللي تبيه 👇',
        footer: totalBatches > 1 ? `الأوامر (${i + 1}/${totalBatches})` : 'الأوامر',
        interactiveButtons: batch.map((b) => ({
          name: 'quick_reply',
          buttonParamsJson: JSON.stringify({ display_text: b.text, id: b.id })
        }))
      });
    } catch (btnErr) {
      // fail-open: زي باقي الأزرار بالبوت - فشل إرسال دفعة أزرار ما يوقف عرض باقي القسم
      console.error(`تعذر إرسال أزرار قسم ${cat} (دفعة ${i + 1}/${totalBatches}):`, btnErr.message);
    }
  }
}
...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  if (stripButtonEmoji(body) === '!عام') {
    ...
    await sendCommandButtons(sock, chatId, persistDir, 'general');
    return;
  }
  ...
  if (stripButtonEmoji(body) === '!العاب') {
    ...
    await sendCommandButtons(sock, chatId, persistDir, 'games');
    return;
  }
  ...
  if (stripButtonEmoji(body) === '!ادارة') {
    ...
    await sendCommandButtons(sock, chatId, persistDir, 'admin');
    return;
  }
