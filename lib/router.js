...
const greetingReplies = require('./greeting-replies'); // 🆕 ردود تلقائية عشوائية على التحايا العادية - ملف مستقل بالكامل
...
const COMMAND_GATES = [
  ...
  { key: 'صورة', test: (b) => b.startsWith('!صورة') },
...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  // 🆕 كشف تلقائي لأي رسالة "تظهر مرة وحدة" (صورة/فيديو/صوت) وحفظ نسخة منها
  // بالذاكرة عشان نقدر نرجعها لاحقاً بأمر !كشف. ما بتوقف المعالجة العادية -
  // الرسالة "تظهر مرة وحدة" أصلاً بلا نص أمر، فباقي الكود تحت بيتجاهلها بهدوء.
  if (!fromMe) {
    await viewOnceWatch.tryCaptureViewOnce(msg, chatId, authorId, wa);
  }
  ...
  // -------- 🆕 ردود تلقائية عشوائية على التحايا العادية (سلام/صباح/تصبحو/وداع) --------
  if (isGroup && !fromMe && body && !body.startsWith('!')) {
    const greetingReply = greetingReplies.getReply(body);
    if (greetingReply) {
      await wa.reply(sock, msg, greetingReply);
      return;
    }
  }
  ...
  // -------- !صورة [رد على استيكر، أو ابعته مع الأمر] - عكس !استريك --------
  if (body.startsWith('!صورة')) {
    try {
      let downloaded = wa.getMediaType(msg) === 'sticker' ? await wa.downloadMedia(msg) : null;
      if (!downloaded && wa.hasQuotedMessage(msg)) {
        const quotedInfo = wa.getQuotedInfo(msg);
        downloaded = await wa.downloadQuotedMedia(quotedInfo.message);
      }
      if (!downloaded) {
        await wa.reply(sock, msg, 'ابعت استيكر مع الأمر، أو رد (Reply) على استيكر بـ !صورة 🖼️');
        return;
      }

      const imageBuffer = await media.stickerBufferToImageBuffer(downloaded.buffer);
      await sock.sendMessage(chatId, { image: imageBuffer }, { quoted: msg });
    } catch (err) {
      console.error('خطأ بتحويل الاستيكر لصورة:', err.message);
      await wa.reply(sock, msg, `ما قدرت أحول الاستيكر لصورة هلق 😅\n🔧 تفاصيل الخطأ: ${err.message}`);
    }
    return;
  }

  // -------- !كشف [رد على رسالة "تظهر مرة وحدة"] - يرجع الصورة/الفيديو/الصوت --------
  if (body.startsWith('!كشف')) {
    try {
      if (!wa.hasQuotedMessage(msg)) {
        await wa.reply(sock, msg, 'رد (Reply) على الرسالة اللي "تظهر مرة وحدة" (🔒) بـ !كشف عشان أطلعهالك 👀');
        return;
      }

      const quotedInfo = wa.getQuotedInfo(msg);
      const revealed = viewOnceWatch.getCaptured(chatId, quotedInfo.stanzaId);
      if (!revealed) {
        await wa.reply(sock, msg, 'ما لقيت نسخة محفوظة من هاي الرسالة 😅 (يمكن ما كانت "تظهر مرة وحدة" أصلاً، أو انتهت صلاحية الحفظ)');
        return;
      }

      let sendPayload = null;
      if (revealed.mediaType === 'image') sendPayload = { image: revealed.buffer };
      else if (revealed.mediaType === 'video') sendPayload = { video: revealed.buffer };
      else if (revealed.mediaType === 'audio' || revealed.mediaType === 'ptt') {
        sendPayload = { audio: revealed.buffer, mimetype: revealed.mimetype || 'audio/ogg; codecs=opus', ptt: true };
      }

      if (!sendPayload) {
        await wa.reply(sock, msg, 'صيغة الملف مش مدعومة للكشف حاليًا 😅');
        return;
      }

      await sock.sendMessage(chatId, sendPayload, { quoted: msg });
    } catch (err) {
      console.error('خطأ بكشف رسالة تظهر مرة وحدة:', err.message);
      await wa.reply(sock, msg, 'صار خطأ وأنا بحاول أكشف الرسالة 😅');
    }
    return;
  }
